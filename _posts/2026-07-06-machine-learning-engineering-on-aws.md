...
## Module 7: 모델 평가
긍정 : 옳게 판단을 함 (모델이) - TP : 정답을 정답이라 함. FP : 오답을 정답이라 함
부정 : 아니라고 판단을 함 (모델이)
1. 혼돈행렬
	1. 정밀도 : TP / (TP + FP)
		1. 재현율 : TP / (TP + TN) : 실제 긍정 결과가 올바르게 예측 되는 비율 - 참 부정을  최소화. 암을 정상이라고 오진하는 것 같은 것 
## Module 8: 모델 배포 전략
롤링배포
블루그린배포
카나리아배포
1. 실시간 
2. 서버리스 : 람다. - 콜드 스타트가 가능한 워크로드, 예측 불가능은 트래픽, CPU만 가능
3. 비동기식 : 대기열. 거의 실시간, 대규모 요청 및 긴 처리시간이 필요할 때
4. 배치 : 대규모 데이터 세트 처리, 더 높은 처리량, 이벤트 기반 혹은 예약 호출
