...
- **1~2번** — "끝까지 자율적으로 완수하라"는 동일한 **오케스트레이션 지시문**(맨 아래 섹션)을 공유하고, 앞부분의 Task만 다름.
...
**용도**: 프로젝트 곳곳에 중복·일회성으로 만들어진 UI [컴포넌트](/posts/component/), 스타일 토큰, 반복 인터랙션을 하나의 재사용 가능한 디자인 시스템으로 리팩토링하는 대규모 자동화 작업.
...
- [컴포넌트](/posts/component/) — 1번 프롬프트가 통합하려는 흩어진 UI 컴포넌트/라이브러리의 단위
- [interface-design-claude-code](/posts/interface-design-claude-code/) — Claude Code로 인터페이스를 설계하는 디자인 스킬. 이 프롬프트 모음과 같은 결의 작업
- [emilkowalski-design-skills](/posts/emilkowalski-design-skills/) — 인터랙션·상태·마이크로 디테일을 챙기는 디자인 스킬 모음 (6·8번 프롬프트와 통함)
- [tds-react-native-docs](/posts/tds-react-native-docs/) — 토스 디자인 시스템 문서. 1번이 지향하는 통합 컴포넌트 라이브러리의 실제 사례
