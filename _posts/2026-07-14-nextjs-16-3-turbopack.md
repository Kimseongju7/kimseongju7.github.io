...
description: 'Next.js 16.3 프리뷰의 Turbopack 업데이트 정리. 이번 안정 릴리스는 컴파일러 성능에 집중해 dev 서버 메모리를 최대 90% 줄이고, 빌드용 영속 파일시스템 캐시와 실험적 Rust React Compiler, import.meta.glob API를 도입했다.'
---
...
- Vercel의 대형 Next.js 앱에서 성능을 분석해 HMR 구독을 더 효율적으로 만들었다. 페이지에 로드된 청크 추적을 여러 구독에서 단일 구독으로 합치는 변경으로, 복잡한 앱의 dev 서버 [콜드 스타트](/posts/cold-start/)를 15% 이상 단축했다. 향후 릴리스에서 추가 메모리·콜드 스타트 개선이 예고돼 있다.
...
- [js-framework-hype-cycle](/posts/js-framework-hype-cycle/) — Next.js가 지나온 JS 프레임워크 유행사 속에서의 현재 위치
- [메타프레임워크](/posts/meta-framework/) — Next.js를 포함한 메타프레임워크 개념 정리
- [JS번들](/posts/js-bundle/) — Turbopack이 대체하는 자바스크립트 번들러 개념
