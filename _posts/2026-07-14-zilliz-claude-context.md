...
description: 'Claude Context는 코드베이스 전체를 벡터 데이터베이스에 인덱싱해 질의와 관련된 코드만 컨텍스트로 가져오는 시맨틱 코드 검색 MCP 플러그인으로, 동등한 검색 품질에서 약 40%의 토큰을 절감한다.'
---
...
- [code-review-graph](/posts/code-review-graph/) — 코드베이스를 지능형 그래프로 색인해 탐색한다는 유사한 코드 인텔리전스 접근
- [headroom-context-compression](/posts/headroom-context-compression/) — 관련 코드만 컨텍스트로 가져와 토큰을 아끼는, 컨텍스트 절약과 맞닿은 주제
- [openai-codex-plugin-cc](/posts/openai-codex-plugin-cc/) — 이 MCP 서버를 Codex CLI(`mcp_servers`)에 붙일 때 참고할 Codex 플러그인
