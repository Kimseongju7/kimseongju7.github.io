...
- **네이티브 [멀티테넌시](/posts/multitenancy/)**: 조직(organization)과 스트림(stream)이 일급 개념, 완전한 데이터 격리
...
- HA([고가용성](/posts/high-availability/)) 모드는 페타바이트급 워크로드까지 확장
...
- [Amazon CloudWatch](/posts/amazon-cloudwatch/) — AWS의 관측성/모니터링 서비스
- [멀티테넌시](/posts/multitenancy/) — 조직·스트림 단위 데이터 격리의 기반 개념
- [고가용성](/posts/high-availability/) — HA 모드가 지향하는 무중단 운영 속성
- [bytedance-deer-flow](/posts/bytedance-deer-flow/) — OpenTelemetry(Monocle) 기반 에이전트 관측을 지원하는 하네스
