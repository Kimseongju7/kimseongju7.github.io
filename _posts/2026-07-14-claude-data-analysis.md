...
description: 'Claude Code의 서브에이전트·슬래시 커맨드·훅으로 구축한 지능형 데이터 분석 플랫폼 claude-data-analysis 소개.'
---
...
- **시각화 유형**: 종합 대시보드, 트렌드(시계열·이동평균), 분포(히스토그램·박스플롯·밀도), 상관([히트맵](/posts/heatmap/)·산점도), 비교(막대·그룹·스몰 멀티플)
...
- [claude-code-multi-agent-sessions](/posts/claude-code-multi-agent-sessions/) — Claude Code 서브에이전트 멀티세션 활용
- [alirezarezvani-claude-skills](/posts/alirezarezvani-claude-skills/) — 데이터·분석 스킬을 포함한 대규모 스킬 라이브러리
