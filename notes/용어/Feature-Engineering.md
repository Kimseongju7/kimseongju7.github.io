...
- [[상관관계행렬]] — 피처 선택(상관 높은 피처 고르기, 중복 피처 제거)에 쓰는 도구
- [[열지도]] — 상관관계행렬 등 피처 탐색 결과를 시각화하는 기법
- [[Machine Learning Engineering on AWS]] — 훈련 전 데이터 준비 단계의 하나로 등장
- [[차원의-저주]] — 피처 선택·차원 축소가 대응하는 문제 상황
- [[서수 인코딩]] — 변환(transformation) 단계에 속하는 범주형 인코딩 기법의 하나
