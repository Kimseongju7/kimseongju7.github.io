...
[[우로보로스 사용법|Ouroboros]]의 `ModelRouter`는 3개 티어를 둔다.
...
단계별 오버라이드도 있다. 예를 들어 [[측면 사고]] 호출은 기본이 `frontier`인데, 창의적 재구성에는 애초에 높은 모델 능력이 필요하다고 보기 때문이다. 반대로 인터뷰 명료화(clarification) 단계는 별도 티어를 지정할 수 있다.
...
## 관련
- [[Seed 스펙]] — PAL이 실행하는 대상(AC)이 정의된 곳
- [[측면 사고]] — 기본 티어가 `frontier`로 지정된 대표 단계
- [[우로보로스 사용법]] — `--efficiency-mode`, `--frugality-assurance` 등 관련 플래그
- [[ouroboros-agent-os|Q00/ouroboros — 프롬프트를 멈추고 명세를 시작하는 Agent OS]] — 런타임·라우팅 계층 설명
- [[Loop Engineering]] — 루프에서 "토큰 폭발"을 막는 문제의식과 직접 연결되는 개념
