...
- **Performance** — [[Virtual DOM|불필요한 리렌더]] 등 성능 안티패턴
- **Architecture** — 구조적 문제
...
## 관련
- [[Loop Engineering]] — "AI가 짠 코드를 사람 대신 검증한다"는 점에서 루프의 *Verification(검증) / 평가자(Evaluator)* 역할과 같은 문제의식
- [[Virtual DOM]] — Performance 카테고리가 잡아내는 불필요한 리렌더는 가상 DOM 재조정 비용과 직결된 문제
- [[컴포넌트]] — React 컴포넌트의 상태·이펙트·구조 패턴을 검사 대상으로 삼는 스캐너
