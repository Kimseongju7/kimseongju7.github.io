...
핵심 기능은 **알람(Alarm)** 이다. 특정 지표가 일정 기간 동안 임계치를 넘으면(예: CPU 80% 이상 5분 지속) 알람이 `ALARM` 상태로 바뀌고, 이걸 방아쇠로 다른 동작을 건다. 대표적으로 SNS로 알림을 보내거나, **[[Amazon-EC2-Auto-Scaling|Auto Scaling]]** 을 발동해 인스턴스를 늘리고/줄인다. 이 덕분에 부하에 맞춰 자원을 자동으로 [[프로비저닝]]하며 [[고가용성]]을 유지하는 흐름의 중심에 CloudWatch가 있다.
...
## 관련
- [[Amazon-EC2-Auto-Scaling]] — CloudWatch 알람이 방아쇠가 되어 인스턴스를 늘리고 줄이는 짝 서비스
- [[사전강의 - AWS Cloud Practitioner Essentials]] — CloudWatch 개념이 처음 등장한 강의 정리 노트
- [[고가용성]] — CloudWatch의 알람·Auto Scaling 연계로 유지하는 목표
- [[프로비저닝]] — CloudWatch 알람이 촉발하는 자원 준비 과정
- [[온디맨드]] — 필요한 만큼만 자원을 쓰게 하는, CloudWatch가 뒷받침하는 과금 방식
