...
인터넷에서 주운 디자인 시스템 관련 프롬프트 모음. 출처는 [[2026-06-29]] 일지에서 분리.
...
- [[히어로]] — 7번 랜딩 페이지 프롬프트가 배치하라는 첫 화면(가치 제안 + CTA) 영역
- [[Loop Engineering]] — 1·2번의 "병렬 에이전트 분배→검증→리뷰" 오케스트레이션 지시문이 루프의 Five Moves와 같은 구조
- [[브레이크포인트]] — 2번 접근성·멀티 디바이스 프롬프트가 점검하는 반응형(데스크톱/태블릿/폰) 전환 기준
- [[컴포넌트]] — 1번 프롬프트가 통합하려는 흩어진 UI 컴포넌트/라이브러리의 단위
- [[interface-design-claude-code]] — Claude Code로 인터페이스를 설계하는 디자인 스킬. 이 프롬프트 모음과 같은 결의 작업
- [[emilkowalski-design-skills]] — 인터랙션·상태·마이크로 디테일을 챙기는 디자인 스킬 모음 (6·8번 프롬프트와 통함)
- [[tds-react-native-docs]] — 토스 디자인 시스템 문서. 1번이 지향하는 통합 컴포넌트 라이브러리의 실제 사례
