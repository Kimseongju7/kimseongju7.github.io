...
slug: ouroboros-usage
publish: true
---
[[ouroboros-agent-os|Ouroboros]] 플러그인을 실제로 쓰면서 정리한 명령·플래그·운영 요령. 무엇인지에 대한 설명은 정리본 노트에 있고, 여기는 **내 환경에서 어떻게 굴리는가**만 다룬다.
...
**요구사항 [[결정화]](crystallization) 엔진.** 애매한 아이디어를 즉흥 프롬프트로 때우지 말고, 구조화된 워크플로로 검증된 코드까지 끌고 간다.

핵심 5요소:

1. **[[소크라테스 인터뷰]]** — 숨은 가정을 질문으로 드러냄
2. **[[Seed 스펙|Seed]] 생성** — 불변 YAML 스펙 생성
3. **[[PAL 라우팅]]** — 작업 복잡도에 따라 모델 자동 승급/강등
4. **[[측면 사고]](lateral thinking)** — 5개 페르소나로 정체 돌파
5. **3단 평가** — 기계적 → 의미적 → 합의
...
| 개념 | 설명 |
|---|---|
| **[[Seed 스펙\|Seed Spec]]** | 인터뷰에서 자동 생성되는 **불변** YAML. 프로젝트 설명 + AC 트리 + 제약 + 아키텍처 결정. "무엇을 만들지"의 유일한 진실. `~/.ouroboros/seeds/seed_<id>.yaml` |
| **[[AC 트리]]** | Acceptance Criteria 계층 분해. 각 AC 상태: `pending` / `in_progress` / `passed` / `failed`. 진행률 추적 단위 |
...
이 다섯은 **상태 없는 사고방식 렌즈**다([[측면 사고]]). evaluator·ontologist 같은 상태 있는 역할은 이 풀에 안 섞이고 각자 SKILL이 있다.
...
| 에이전트 | 역할 |
|---|---|
| `ouroboros:socratic-interviewer` | 질문으로 숨은 가정 노출 ([[소크라테스 인터뷰]]) |
| `ouroboros:ontologist` | 증상 말고 근본 문제 찾기 |
| `ouroboros:seed-architect` | 요구사항을 [[Seed 스펙]]으로 [[결정화]] |
| `ouroboros:evaluator` | 3단 검증 |
| `ouroboros:contrarian` / `hacker` / `simplifier` / `researcher` / `architect` | [[측면 사고]] 5인 |
...
| `config-reference.md` | 설정 키·기본값 ([[PAL 라우팅]] 티어 정의 포함) |
...
| `README.ko.md` (루트) | 한국어 README ([[ouroboros-readme-ko\|정리본]]) |
...
- [[결정화]] · [[소크라테스 인터뷰]] · [[Seed 스펙]] · [[AC 트리]] · [[PAL 라우팅]] · [[측면 사고]] — 핵심 개념 용어 노트
- [[ouroboros-agent-os|Q00/ouroboros — 프롬프트를 멈추고 명세를 시작하는 Agent OS]] — 영문 README 정리본
- [[ouroboros-readme-ko|Ouroboros 한국어 README — 명세 우선 AI 개발 시스템]] — 한국어판 고유 내용
- [[Caveman 사용법]] — 같은 성격의 플러그인 운영 메모
- [[Loop Engineering]] — 이 워크플로가 속한 더 큰 그림
